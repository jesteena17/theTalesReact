...
export default class MainMenu extends Component {
    ...
    render() {
// dirct destruture
        const result=this.state.sections.map(({id,title,imageUrl,subtitle,size})=>{
            return (<MenuItem key={id} id={id} title={title} subtitle ={subtitle} imageUrl={imageUrl} size={size} />);
        })
        return (
